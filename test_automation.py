...
DEFAULT_EXCEL_CANDIDATES = [
    str(TESTS_DIR / "Assignment 1 - Test cases IT23750906.xlsx"),
]
...
# Module-level flushing print so output is visible in real-time on Windows
...
def _find_column_index(header_values: list, requested_name: str | None, candidates: list[str]) -> int | None:
    ...
    def match(name: str) -> int | None:
        ...
        for i, v in indexed:
            nv = _normalize_header(v)
            # Only allow substring match if query covers most of the header
            # This prevents "singlish" matching "singlishinputtypescovered"
            if n in nv and len(n) >= len(nv) * 0.6:
                return i
            if nv in n and len(nv) >= len(n) * 0.6:
                return i
        return None
...
def _find_chat_locators(page, timeout_ms: int):
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if input_by_ph.count() > 0 and output_by_ph.count() > 0 and input_by_ph.is_visible() and output_by_ph.is_visible():
                ...
                clear_btn = page.get_by_role("button", name=re.compile(r"Clear", re.IGNORECASE)).first
                return input_by_ph, output_by_ph, action, clear_btn
        except Exception:
            pass
        ...
        try:
            ...
            if len(visible) >= 2:
                action = page.get_by_role("button", name=re.compile(r"^Transliterate$", re.IGNORECASE)).first
                clear_btn = page.get_by_role("button", name=re.compile(r"Clear", re.IGNORECASE)).first
                return visible[0], visible[1], action, clear_btn
    ...
    try:
        ...
        _print("Debug: textarea meta:", meta)
    except Exception as e:
        _print("Debug: failed to read textarea meta:", e)
    if last_debug:
        _print("Debug: last error:", last_debug)
    raise RuntimeError("Could not find Chat UI locators (input/output textareas).")

def _has_fetch_error(page) -> bool:
    """Check if the page shows a 'Failed to fetch' error."""
    try:
...
def run_test():
    ...
    _print(f"Starting Frontend-Only test with {rows_total} rows...")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            _print("Frontend loaded successfully.")
        except Exception as e:
            _print(f"Error loading frontend: {e}")
            browser.close()
            return
        ...
        if is_chat:
            try:
                input_locator, output_locator, action_locator, clear_locator = _find_chat_locators(page, int(args.timeout_ms))
            except Exception as e:
                _print(f"Error locating chat UI elements: {e}")
                browser.close()
                return
        else:
        ...
        for row_index in range(header_row + 1, int(ws.max_row or 0) + 1):
            ...
            _print(f"Testing [Row {row_index}]: {singlish_input}")

            try:
                _dismiss_overlays(page)

                # Step 1: Use the Clear button to reset both input and output cleanly
                if clear_locator:
                    try:
                        clear_locator.click(timeout=3000)
                        page.wait_for_timeout(500)
                    except Exception:
                        pass

                # Step 2: Type the input and click Transliterate
                _ensure_input_value(page, input_locator, singlish_input, int(args.type_delay_ms))

                # Step 3: Click Transliterate with retry on "Failed to fetch"
                max_api_retries = 3
                api_success = False
                for attempt in range(max_api_retries):
                    ...
                    if _has_fetch_error(page):
                        _print(f"  [Retry {attempt+1}/{max_api_retries}] Failed to fetch, retrying...")
                ...
                # Step 4: Wait for non-empty output with stabilization + retry
                actual_output = ""
                output_retries = 3  # Total attempts to get output
                for out_attempt in range(output_retries):
                    stable_count = 0
                    last_seen = ""
                    max_total_wait_s = (max(1, int(args.retries)) * max(0, int(args.retry_wait_ms)) / 1000)
                    deadline = time.time() + max_total_wait_s
                    ...
                    if not actual_output and last_seen:
                        actual_output = last_seen

                    if actual_output:
                        break

                    # Output is still empty — retry Transliterate
                    if out_attempt < output_retries - 1:
                        _print(f"  [Output empty, re-clicking Transliterate {out_attempt+2}/{output_retries}]")
                        if action_locator:
                            action_locator.click()
                        page.wait_for_timeout(max(0, int(args.wait_ms)))
                ...
                _print(f"Error in UI interaction: {e}")
                try:
        ...
        if args.keep_open and not args.headless:
            try:
                ...
                pass
            _print("Keeping browser open. Press CTRL+C to stop.")
            try:
                while True:
    ...
    try:
        ...
        _print(f"Error saving output file '{args.output}': {e}")
        return

    _print(f"Test completed. Results saved to {args.output}")
